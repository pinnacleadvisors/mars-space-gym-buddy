...
**🔧 Schema Management**:
- **Schema Definition**: Defined directly in Supabase Dashboard (Tables, Functions, RLS Policies)
...
- **No Migration Files**: Schema changes are managed directly in Supabase; `database.ts` is the canonical reference
...
- **Primary**: `src/types/database.ts` - **Source of truth** for database schema (auto-generated, updated on every commit to `main`)
  ...
  - **No migration files**: Schema is managed directly in Supabase Dashboard; `database.ts` reflects the current state
...
- **Status**: Fixed - function is defined in Supabase with proper security settings
...
- **Cause**: Supabase needs explicit foreign key relationship syntax in select queries
- **Solution**: Use `class_sessions(...)` syntax in select queries (the foreign key `class_bookings.class_id` → `class_sessions.id` is correctly defined)
- **Example**: `.select('*, class_sessions(id, name, instructor, ...)')`
- **Note**: The foreign key is correctly defined in the database; verify the relationship in `src/types/database.ts` under the table's `Relationships` array
