...
const Register = () => {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: getFullRedirectUrl('/auth/callback'),
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
          },
      ...
      // Note: If successful, user will be redirected to Google for authentication
      // and then back to /auth/callback which handles the OAuth response
    } catch (error: any) {
      showErrorToast({
        title: "Authentication Error",
      ...
      // Only reset loading if not redirecting (i.e., on error)
      // The redirect will navigate away from this page
      setTimeout(() => setIsGoogleLoading(false), 3000);
    }
  };

  // TODO: Future implementation - Apple Sign In
  ...
  //   });
  //   if (error) {
  //     showErrorToast({ title: "Authentication Error", description: error.message });
  //   }
  // };
      ...
      <Card className="w-full max-w-md shadow-lg">
        ...
        <CardContent>
          ...
          <div className="space-y-3">
            ...
            {/* Apple Sign Up - Future Implementation
            <Button
              type="button"
              variant="outline"
              className="w-full"
